<template>
  <section class="section">
    <div class="container">
      <div class="is-max-w-md mx-auto">
        <div class="mb-6 has-text-centered"><a href="#"></a></div>
        <div class="box has-text-centered py-6 px-5">
          <span class="subtitle is-spaced">Sign Up</span>
          <h4 class="title is-size-3">Create an account</h4>
          <Form
            @submit="handleRegister"
            :validation-schema="schema"
            v-slot="{ errors }"
          >
            <div class="field">
              <div class="field-body">
                <div class="field">
                  <div class="control">
                    <Field
                      class="input"
                      type="text"
                      name="firstName"
                      :class="{ 'is-danger': errors.firstName }"
                      placeholder="First name"
                    />
                    <ErrorMessage class="help is-danger" name="firstName" />
                  </div>
                </div>
                <div class="field">
                  <div class="control">
                    <Field
                      class="input"
                      type="text"
                      name="lastName"
                      :class="{ 'is-danger': errors.lastName }"
                      placeholder="Last name"
                    />
                    <ErrorMessage class="help is-danger" name="lastName" />
                  </div>
                </div>
              </div>
            </div>
            <div class="mb-5">
              <div class="field">
                <div class="control has-icons-right">
                  <Field
                    class="input"
                    type="email"
                    name="email"
                    :class="{ 'is-danger': errors.email }"
                    placeholder="name@email.com"
                  />
                  <ErrorMessage class="help is-danger" name="email" />

                  <span class="icon is-small is-right">
                    <svg
                      class="image is-24x24"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207"
                      ></path>
                    </svg>
                  </span>
                </div>
              </div>
              <div class="field">
                <div class="control has-icons-right">
                  <Field
                    class="input"
                    type="password"
                    name="password"
                    :class="{ 'is-danger': errors.password }"
                    placeholder="Enter your password"
                  />
                  <ErrorMessage class="help is-danger" name="password" />
                  <span class="icon is-small is-right">
                    <svg
                      class="image is-24x24"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"
                      ></path>
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"
                      ></path>
                    </svg>
                  </span>
                </div>
              </div>
              <div class="field">
                <div class="control">
                  <button type="submit" class="button is-primary is-fullwidth">
                    Sign Up
                  </button>
                </div>
              </div>
            </div>
          </Form>
        </div>
        <div class="has-text-centered">
          <p><a href="#">Police privacy</a> and <a href="#">Terms of Use</a></p>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
import { Form, Field, ErrorMessage } from "vee-validate";
import * as yup from "yup";
export default {
  components: {
    Form,
    Field,
    ErrorMessage,
  },
  data() {
    const schema = yup.object({
      firstName: yup.string().required("First Name is required"),
      lastName: yup.string().required("Last Name is required"),
      email: yup
        .string()
        .required("Email is required")
        .email("Please email valid email address"),
      password: yup.string().required("Password is required").min(8),
    });

    return {
      schema,
    };
  },
  methods: {
    handleRegister(values) {
      console.log(values);
    },
  },
};
</script>

<style>
.help.is-danger {
  display: flex;
}
</style>
